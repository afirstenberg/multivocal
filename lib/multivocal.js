...
var preprocess = function( env ){
  ...
  return timingBegin( env, 'Preprocess' )
    .then( env => loadConfig( env ) )
    .then( env => buildEnvPlatform( env ) )
    .then( env => preprocessGooglePing( env ) )
...
var formatMessageTemplate = function( env, source, target ){
  var val = Util.setting( source, `Template`, Template.Methods.Str );
  if( typeof val !== 'undefined' ){
...
var formatPage = function( env ){
  ...
  if( typeof pageUrlStatePath == 'array' ){
  ...
  if( pageUrl && pageData ){
    ...
    // TODO - supplement Data with some standard state
