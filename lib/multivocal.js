...
var buildEnvParameters = function( env ){
  ...
  Object.keys( env.ParameterInfo ).forEach( param => {
    ...
    if( typeof val === 'object' ){
      ...
      val = Util.objPath( val, valuePaths );
    }
    if( val ){
...
var formatTypeValues = function( name, values ){
...
var formatType = function( name, values, env ){
  ...
  var entities = valueNames.map( valueName => formatTypeValues( valueName, values[valueName] ) );
