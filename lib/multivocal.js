...
new exports.Config.KeyValue( process.env, {root:'Process/Env'} );
...
var preprocessVerifyRequestJWT = function( ruleName, env ){
  ...
  return timingBegin( env, `Verify ${ruleName}` )
    .then( env => {
    ...
    .then( ret => {
      return timingEnd( env, `Verify ${ruleName}` )
        .then( env => ret );
    })
}
...
var buildEnvPlatform = function( env ){
  return timingBegin( env, 'Platform' )
    .then( env => {
      var rules = Util.setting( env, 'Platform/RuleCriteria' );
      Object.keys( rules ).forEach( ruleName => {
        //timingBegin( env, `Platform ${ruleName}` );
        ...
        //timingEnd( env, `Platform ${ruleName}` );
      });

      console.log( 'Multivocal buildEnvPlatform', env.Platform );
      return Promise.resolve( env );
    })
    .then( env => timingEnd( env, 'Platform' ) );
};

var buildEnvHostname = function( env ){
...
var handle = function( env ){
  ...
  return timingBegin( env, 'Handle' )
    .then( env => {
      ...
      // Call the handler
      return timingBegin( env, `Handle ${handlerName}` )
        .then( env => handler( env ) )
        .then( env => timingEnd( env, `Handle ${handlerName}`) );
