...
var buildEnvLocale = function( env ){
  var localePath = ['Body/originalRequest/data/user/locale'];
  var locale = Util.objPathsDefault( env, localePath, 'und' );
...
var buildEnvParameters = function( env ){
  var parameterPath = ['Body/result/parameters'];
  env.Parameter = Util.objPathsDefault( env, parameterPath, {} );
  return Promise.resolve( env );
};
...
var buildEnvIntents = function( env ){
  ...
  var intentPath = ['Body/result/metadata/intentName'];
  env.Intent = 'Intent.'+Util.objPathsDefault( env, intentPath );

  var actionPath = ['Body/result/action'];
  env.Action = 'Action.'+Util.objPathsDefault( env, actionPath );
...
var loadVoice = function( env ){
  ...
  var voicesPath = [
    `Config/Local/${env.Locale}/Voice`,
    `Config/Local/${env.Lang}/Voice}`,
  ...
  var voices = Util.objPathsDefault( env, voicesPath, voicesDefault );
...
var localePaths = function( env, branches, leaves ){
  var ret = [];

  var locales = [env.Locale, env.Lang, 'und'];
  if( !Array.isArray(branches) ){
    branches = [branches];
  }
  if( typeof leaves === 'undefined' ){
    leaves = [ env.Outent, env.Intent, env.Action, 'Default' ];
      ...
      for( var branchIndex=0; branchIndex<branches.length; branchIndex++ ){
        ...
        if( branch ){
          for( var leafIndex=0; leafIndex<leaves.length; leafIndex++ ){
            ...
            if( leaf ){
              var path = `Config/Local/${locale}/${branch}/${leaf}`;
              ret.push( path );
            }
          }
        }
...
var handleDefault = function( env ){
  var responseNames = localePaths( env, 'Response' );
...
var addSuffix = function( env ){
  ...
  var responseNames = localePaths( env, 'Suffix' );
