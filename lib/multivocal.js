...
var buildEnvContexts = function( env ){
  ...
  for( var co=0; contexts && co<contexts.length; co++ ){
...
var sendMessageContent = function( env, envName, template ){
  ...
  var repeatVal = Util.setting( env, `${envName}/Repeat`, Template.Methods.Str );
  if( repeatVal ){
    Util.setObjPath( env, 'Send/Remember/name', 'multivocal_repeat' );
    Util.setObjPath( env, 'Send/Remember/lifespan', 1 );
    Util.setObjPath( env, `Send/Remember/parameters/${envName}`, repeatVal )
  }
...
var JsonBuilder = require('./builder')(require('../config/build-aog2-dialogflow2.js'));
