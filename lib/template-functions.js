...
helper.Ssml = function( voice, options ){
  ...
  var out = '<speak>';
  out += openTag( 'voice',    voice && voice.Voice );
  out += openTag( 'prosody',  voice && voice.Prosody );
  out += options.fn( options.data.root );
  out += closeTag( 'prosody', voice && voice.Prosody );
  out += closeTag( 'voice',   voice && voice.Voice );
  out += '</speak>';
...
helper.FirstVal = function( paths /*...*/ ){
  ...
  var options = optionArgument( paths );
