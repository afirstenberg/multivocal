...
async function FlexResponse( env ){
  const targets = Util.setting( env, 'FlexResponse/Targets' )
  for( let co=0; co<targets.length; co++ ){
    const target = targets[co]
    env['_Target'] = target
    env = await Response.getFromSettings( env, target )
  }
  return env
}
...
async function HandleDefault( env ){
  ...
  if( !env.Handled && handlers['Default'] ){
    env = await (handlers['Default'])(env) || env
  }
  return env
}
handleProcessors.add( HandleDefault )

async function Suffix( env ){
  ...
  const noSuffixNeeded = Util.setting( env, 'NoSuffixNeeded/Criteria', Template.Methods.Bool )
  ...
  return await Response.getFromSettings( env, 'Suffix' )
}
handleProcessors.add( Suffix )
