...
var buildMessages = function( env, response, templateResultMap ){
  ...
  Object.assign(
    ret,
    executeMessages( env, response, templateResultMap, Template.eval )
  );
  return Promise.resolve( ret );
};
...
var executeMessages = function( env, response, templateResultMap, templateExec ){
  var ret = {};
  ...
  for( var co=0; co<keys.length; co++ ){
    ...
    if( template !== undefined ){
      ...
      // If the only property of the result object is _This, then we just want
      // the value itself and not the object.
      if( resultValue && Object.keys(resultValue).length === 1 && resultValue['_This'] ){
        resultValue = resultValue['_This'];
      }
