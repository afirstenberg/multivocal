...
var get = function( env, responseNames, responseField, responseDefault, templateResponseMap, rawParameterName, transformers ){
  ...
  return getResponses( env, responseNames )
    ...
    .then( ({responsePath,responseIndex,responses}) => {
      rpath = responsePath;
      rindex = responseIndex;
      return filterResponses( env, responses, responseDefault, rawParameterName );
    })
    ...
    .then( response => {
      env[responseField] = objectAssignDeep( {}, response );
      if( response.Debug ){
        env[responseField].Debug.Path = rpath;
        env[responseField].Debug.PathIndex = rindex;
      }
...
var getResponses = function( env, responseNames ){
  ...
  for( var co=0; co<responseNames.length && !ret; co++ ){
    var path = responseNames[co];
    var responses = Util.objPath( env, path );
    if( responses ){
      ret = {
        responsePath: path,
        responseIndex: co,
  ...
  if( !ret ){
    ret = {};
    console.error( 'No responses found for names: ', responseNames );
  }
...
var envToSpeechMarkdownPlatform = function( env ){
  return Util.objPath( env, 'Platform/Markdown' );
}

var envToSpeechMarkdownVoices = function( env ){

  var isTag = function( s ){
  ...
  var voices = env.Voices;
  var names = Object.keys( voices );
...
var speechMarkdownTransformer = function( val, env ){
  if( val && (typeof val === 'object') && val.Markdown ){
    ...
    const platform = envToSpeechMarkdownPlatform( env );
    const voices = envToSpeechMarkdownVoices( env );
    const options = {
      platform: platform,
