...
var getFromSettings = function( env, settingPath ){
  ...
  var transformers =
    ...
    Util.setting( env, 'FlexResponse/Transformers');
...
var sanitizeSsml = function( text ){
  if( text ){
    text = text.replace( /&/g, '&amp;' );
    text = text.replace( /</g, '&lt;' );
    text = text.replace( />/g, '&gt;' );
...
var simpleTextToSsmlTransformer = function( val, env ){
  if( val && (typeof val === 'object') && val.Text && !val.Ssml ){
    val.Ssml = sanitizeSsml( val.Text );
  }
  return val;
}
...
var speechMarkdownTransformer = function( val, env ){
  if( val && (typeof val === 'object') && val.Markdown ){
    ...
    if( !val.Ssml ){
      var ssmlmd = markdown;
      var sanitize = Util.setting( env, 'SpeechMarkdown/SanitizeSsml' );
      if( sanitize ){
        ssmlmd = sanitizeSsml( markdown );
      }
  ...
  for( var co=0; co<keys.length; co++ ){
    ...
    if( template !== undefined ){
      ...
      transformers.forEach( transformerName => {
        ...
        if( func ){
          resultValue = func( resultValue, env );
        } else {
