...
var updateSourceKeys = function( source, env ){
  ...
  return Timing.begin( env, "Auth updateSourceKeys" )
    ...
    .then( response => {
      ...
      Timing.end( env, "Auth updateSourceKeys" );
...
var getUnverifiedInfo = function( token, env ){
  ...
  return Timing.begin( env, "Auth getUnverifiedInfo" )
    ...
    .then( audOkResult => {
      ...
      Timing.end( env, "Auth getUnverifiedInfo" );
      return Promise.resolve( ret );
    });
};

var verifyInfo = function( info, env ){
  //Timing.begin( env, 'Auth verifyInfo' );
  ...
  return new Promise((resolve,reject) => {
    Jwt.verify( token, key, options, function( err, decoded ){
      //Timing.end( env, 'Auth verifyInfo' );
      if( err ){
        reject( err );
      } else {
        resolve( decoded );
